...
orderItems.forEach((item, index) => {
    ...
    let supplierProductNumber = vpn === "" || item.status === "05" ? item.productApn : vpn
    let expectedCostPrice = item.costBeforeTax;
    let descriptions = item.productDescription.match(/.{1,35}/g) || [""];

    //if description is empty then remove the ItemDescription segment
    if(descriptions === ""){
        targetJSON.LineItems.push({
            ...
            AdditionalProductID: {
                FunctionQualifier: "1",
                ItemNumberIdentification: {
                    ItemNumber: supplierProductNumber,
